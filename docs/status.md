...
change anything" instruction**: the remaining Medium/Low items above
(Google-sign-in surname mis-mapping, unguarded `updateProject` overwrite,
misc blog corruption, `ProjectData` size-cap inconsistency, `floodRisk`
cache-TTL order-dependency, designer ID-collision risk), the two blog
High-severity content items (dangling table/list references, unsourced
medical claims), plus the ~211-of-236-posts-still-unchecked blog-corpus
