...
| Build a real AuthContext for the web app | Codebase audit — Section A | web | Must | Done 2026-08-24. `swales-designer` already had a working `context/AuthContext.tsx` (this item's gap was specifically `swales-services`, which didn't). Built `swales-services/src/context/AuthContext.jsx` mirroring designer's pattern, adapted to services' existing conventions (`accessToken` localStorage key, `NEXT_PUBLIC_API_BASE_URL` + `/api`), wired into `providers.js`, and hooked into `login/page.js` so `user` state updates without a hard reload. Caught and fixed a real regression along the way: the first draft copied designer's `{!isLoading && children}` render gate, which silently blanked Header/nav content out of every statically-prerendered page (verified via a before/after build comparison) — fine for designer's app-like SPA, wrong for this SEO-dependent marketing/blog site. Fixed by always rendering `children`. Verified end-to-end against the live rebuild backend: an invalid token correctly triggers `/api/auth/me` → 401 → self-clearing `logout()`. `Header.jsx` (the actually-rendered header) keeps its own independent auth-state logic — not migrated to the new context, out of scope for this item. |
...
| Decide monetization framework (Free / free-for-contact / Core paid / Post-core) | Codebase audit — Section D | both | Must | **Decided 2026-08-24.** Priestley 4-product ladder, reconciled against what's actually already built (verified in code, not assumed): **Free** — all analysis maps (solar/wind/soil/water stress/altitude/contour/flooding/weather) + the design canvas, no gate, no account needed. **Free-for-contact** — printing/downloading a report or a design requires an account; this stage already exists and works today (`ReportAuthGateModal` + the combined Site Report PDF generator in `swales-services`) — no build needed. **Core paid** (monthly + yearly subscription, price points not yet set) — at launch, this is an *enhanced* version of the report (multi-location comparison, unlimited regenerations, no upsell footer) — deliberately not just re-gating the same free report behind a paywall. Saved-project-limit enforcement, real multi-user collaboration (beyond today's single-link sharing), the planting-recommendation AI advisor, and the not-yet-built "consulting site details for energy" feature are explicitly **deferred, not part of the initial paid launch** — Omar's call, to avoid building a full feature set before a single subscriber exists. **Post-core**: intentionally undefined — to be designed once there's real usage data. Blocked on Stripe actually being wired for real subscriptions, which is itself blocked on Omar opening a business bank account (tracked separately). See `status.md` for the full reasoning, including what was verified already-built vs. assumed. |
