...
- **`docs/roadmap.md`** (this file) — the stable plan: phases, ordering, reasoning. Canonical, lives only here (`back/` repo). `designer/` and `services/` each carry a one-line pointer to it in their own `CLAUDE.md`, not a copy.
...
| Push pending local-only planning docs (`CLAUDE.md`, `docs/roadmap.md`, `docs/status.md`, `docs/roadmap_backlog.xlsx`) in all 3 repos | Must | Not started | Currently sitting uncommitted locally in `back`/`designer`/`services` as of this session. Nothing else across the 3 repos is uncommitted or unpushed — verified with `git status` before this update. |
...
| Replace `sequelize.sync({ alter: true })` with real, reviewable migrations | Must | Not started | [back/server.js:48](../server.js) and [:133](../server.js) auto-alter the live database schema on every server start/deploy, with no review step. This is the concrete mechanism behind "a merge could break the running database" — not a hypothetical risk. |
...
| Fix Google Sign-In: verify the real Google `id_token` server-side | Must | Not started | [back/controllers/socialAuthController.js](../controllers/socialAuthController.js) currently trusts a client-supplied email plus an unverified token string, with no check at all — a live account-takeover hole. Decided approach: verify Google's real `id_token` with `google-auth-library`, over patching the HMAC or adopting a full external auth provider. |
| Create and maintain `back/docs/status.md` as the session-continuity doc | Should | Done | Created this session, imported via `back/CLAUDE.md` alongside this roadmap. Updated at the end of significant sessions going forward. |
...
| Backend: properly verify Google id_token (signature, audience, expiry) | Codebase audit — Section A | backend | Must | The app now sends the real Google id_token, but /api/auth/google-signin still needs to verify it rather than accept any string. Flag to whoever owns that backend service. |
