...
| Field Calculators (folded in: Energy Calculators + new categories) | List B | web | Should | **Hub page + 4 calculators live, 2026-09-13** (`swales-services` `111ddcb`) — independent of both pre-launch gates (plant images, Stripe), no reason to wait. Originally decided 2026-08-27 as earthworks-sizing tools only; Energy Calculators (previously its own separate row) folded in the same day, plus three net-new categories (Vertical Farming, Microgreens, Aquaculture/Aquaponics) kept under this one line rather than given their own roadmap rows. Each calculator is a standalone, manual-entry tool (no map/pin/API dependency — reliability over convenience, and keeps this free tool genuinely separate from the gated map-driven advisors per `feedback_keep_free_tools_separate_from_gated`), using only real, citable formulas, verified by hand before shipping (same bar as RainAdvisor). Full category list and tiered build order in `status.md`'s 2026-09-13 entry. **Tier 1**: swale volume/dimensions (live), mulch volume (live), solar PV array sizing (live), hydro power output (live), terrace spacing/cut-fill (not started), rainwater tank sizing (not started). Tiers 2-4 (battery/inverter sizing, wind output, compost ratio, pond/dam sizing, amendment dosing, ROI/payback, then the data-table-dependent vertical-farming/microgreens/aquaculture calculators, then keyline spacing/biogas/hybrid balancer) — see `status.md` for the full list. Each calculator gets its own SEO-optimized page (`generateMetadata` + JSON-LD schema, same pattern as the blog) plus one shared `/field-calculators` hub/explainer page covering what the tools are, why they matter, and how to measure the inputs. |
