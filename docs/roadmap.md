...
| Field Calculators (folded in: Energy Calculators + new categories) | List B | web | Should | **Tiers 1-2 complete, 2026-09-13 — 13 of 26 calculators live** (`swales-services` `111ddcb`..`940cb69`) — independent of both pre-launch gates (plant images, Stripe), no reason to wait. Originally decided 2026-08-27 as earthworks-sizing tools only; Energy Calculators (previously its own separate row) folded in the same day, plus three net-new categories (Vertical Farming, Microgreens, Aquaculture/Aquaponics) kept under this one line rather than given their own roadmap rows. Each calculator is a standalone, manual-entry tool (no map/pin/API dependency — reliability over convenience, and keeps this free tool genuinely separate from the gated map-driven advisors per `feedback_keep_free_tools_separate_from_gated`), using only real, citable formulas, verified by hand before shipping (same bar as RainAdvisor). **Tier 1** (`64f56dd`): swale volume/dimensions, terrace spacing (bench-terrace VI, not cut-fill — see status.md), mulch volume, rainwater tank sizing, solar PV array sizing, hydro power output. **Tier 2** (`940cb69`): compost C:N ratio, pond/dam volume, soil amendment rate, battery bank sizing, inverter sizing, wind turbine output, solar/wind ROI-payback. **Remaining**: Tier 3 (needs published per-crop/species reference tables — seed rates, DLI targets, nutrient dosing, microgreens seed density, aquaculture stocking density/ratios) and Tier 4 (keyline spacing, biogas, hybrid balancer) — see `status.md` for the full list. Every calculator page ships with `generateMetadata` + `SoftwareApplication`/`FAQPage` JSON-LD schema plus a real SEO/GEO article (explanatory sections + direct-Q&A FAQ list) underneath the tool itself, same pattern as the blog, plus one shared `/field-calculators` hub page covering what the tools are, why they matter, and how to measure the inputs. |
