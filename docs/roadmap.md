...
| Feature | Origin | Platform | Priority | Notes |
|---|---|---|---|---|
| Site comparison (two locations side-by-side: wind/solar/soil) | Codebase audit — Section B | web | Must | **Done, 2026-09-05.** New `/compare` page: one shared mini-map with two pins (A/B toggle — click, "my location", or typed lat/lng all write to whichever pin is active), nothing fetches until the explicit "Compare" click, reusing the main map's `getOrFetch` cache. Gated **Free-for-contact** (sign-in required to run a comparison, same tier/modal as the shareable link and report/design downloads) but the entry point — a new toolbar icon on the main map, next to Share/Print — stays visible to signed-out visitors rather than hidden. Reuses `SolarCard`/`WindDashboard`/`SoilCard` and the existing fetch services as-is, no changes to any of them. Verified live in the browser preview: pin placement fires zero network calls, the auth gate shows comparison-specific copy when signed out, the toolbar entry point is reachable from every layer route, and the layout stacks correctly on mobile. Full design/verification detail in `status.md`'s 2026-09-05 entry. |
...
| RainAdvisor service — Option B (irrigation decisions, flood/runoff risk, swale/water-harvesting sizing) | Codebase audit — Section B | web | Could | **Done, 2026-09-08** — first Core-paid advisory module (see the monetization revision above). Real methodology: SCS/NRCS Curve Number runoff, USDA-NRCS texture-based Hydrologic Soil Group, field-capacity/wilting-point irrigation timing, trapezoidal swale sizing cross-referencing `terrainAnalysis.js`'s existing slope-suitability band. `rainAdvisorService.js` (pure logic) + `RainAdvisor.jsx`, wired into the Precipitation tab below `PrecipitationCard`. Marked with a "Core" pill but not hard-gated yet (no Stripe/subscription-status infra exists — tracked as `future-concerns.md` item 17, must close before real launch) — see `status.md`'s 2026-09-08 entry for full build/verification detail, including a real cross-layer bug found and fixed (`sunLat`/`sunLon` only update on the Sun Tracker layer; `LayerDataPanel` now also gets a layer-agnostic `pinLat`/`pinLng`). |
| Composite soil health score (0-100, weighted, with trend indicator) | Codebase audit — Section B | web | Should | **Already done — discovered 2026-09-13, not built this session.** `SoilCard.jsx`'s hero display already computes a real weighted 0-100 composite (pH 25%, bulk density 20%, organic carbon density 20%, nitrogen 15%, CEC 20%) with a Critical→Excellent level badge and a full explanatory tooltip — live on the free Soil map layer, not gated. This row was carried as "Should — not started" for weeks without anyone checking the actual code; see `status.md`'s 2026-09-13 entry (same class of miss as the 2026-09-03 correction for the PDF report/precipitation card rows). The one piece genuinely missing — a **trend indicator** — isn't buildable honestly: SoilGrids is a static, periodically-modeled dataset, not a live sensor, so there's no real time-series for a single site to show a trend from. |
| Crop suitability engine (ranks wheat/maize/legumes/vegetables/fruit trees with reasons) | Codebase audit — Section B | web | Should | **Done, 2026-09-13** (`swales-services` `7f288c2`) — the genuinely missing half of the pair above. Ranks the 5 named categories against a site's soil pH, drainage (Hydrologic Soil Group, reusing `rainAdvisorService.js`'s existing estimator), and climate zone (reusing `smartClimateAdvisor.js`'s classification) — climate 40% / pH 35% / drainage 25%, each sourced from general agronomy consensus. Wired into the Soil layer below `SoilCard`, same "Core" pill/not-yet-hard-gated pattern as RainAdvisor. Verified live against real Iowa soil data — all 5 scores hand-checked exact. Also fixed while here: `pricing/page.js` was still advertising the already-free soil health score as a Core-paid incentive — corrected. |
...
| Specialized Data Package (GeoJSON/CSV/KML/DXF export, one-time purchase) | New (Omar, inspired by permalogica.com/pricing, 2026-09-12) | web | Must | **Built and priced, 2026-09-12** — `/specialized-reports` page (nav: between Consultations and Blog), $159 one-time. Real export generation (`siteDataExport.js`: GeoJSON, CSV, hand-written KML, DXF via `@tarikjabiri/dxf`) wired against actual analyzed Contour data, verified directly against realistic mock data. **Gating**: enquiry-based/manually fulfilled (no Stripe exists) — accepted by Omar as correct for now, not a gap to close before the bank account/Stripe exist. `ContourAnalysisCard.jsx` shows a locked teaser linking to the request form, doesn't expose working downloads directly. **Also requires a signed-in account to submit the request** (same free-for-contact tier as report/design downloads and Compare) — unlike Consultations, which stays fully open with no account needed. The AI Q&A piece ("Ada" equivalent) — built as the free, ungated, guided (non-LLM) "Ask about this site" panel instead of part of this paid page: `reportQAService.js` + `AskAboutSite.jsx`, 5 topics (Solar/Wind/Irrigation/Swales & Drainage/Overall Suitability), wired layer-agnostically into `LayerDataPanel.jsx`. See `status.md`'s 2026-09-12 entries for full build/verification detail. **Not yet verified**: the teaser's real on-screen behavior after an actual rectangle-draw-and-analyze click-through (map draw tool proved hard to automate this session) — worth a manual check. |
| On-site Energy & Water Consultations page | New (Omar, 2026-09-09) | web | Must | **Built, 2026-09-12** — resolves the "not-yet-built 'consulting site details for energy' feature" mentioned as deferred in the monetization decision above: it's now a standalone **free** page (`/consultations`), deliberately **not** linked to or scoped against Core paid or any gated feature (Omar's explicit call — free tools stay fully separate from gated ones, see `status.md`). 6 energy systems (solar PV siting, small wind turbine siting, off-grid/hybrid design, energy audit, grid-tie vs. off-grid ROI, micro-hydro feasibility) + 7 water systems (swale/earthworks, rainwater harvesting, greywater, pond/dam siting, irrigation design, flood mitigation/drainage, keyline design), each cross-referencing an already-live Swales analysis tool (Solar Potential map, Wind Potential dashboard, RainAdvisor, Contour Analysis, Flood Risk map). Enquiry-only — no price shown, on-site visits by request, quote discussed after contact. Nav order: How it works, Services, Compare, Designer, **Consultations**, Blog, Contact Us (Omar's explicit reorder). See `status.md`'s 2026-09-12 entry for the real `/api/contact-us/message` bug found and fixed while wiring the enquiry form. **Permalogica comparison researched 2026-09-12**: its CAD-export gap is already closed by Specialized Reports below; the one remaining piece (a paid, LLM-backed "Ada"-equivalent Q&A layer, vs. the free rules-based version already shipped) was **decided 2026-09-13 — not now**, kept as a future option under Phase W's AI track whenever that's picked up. See `status.md` for the research and decision. |
...
| Location-based site/observation logging (GPS pins) — the **"Observer" tab** | New (mobile ask) | mobile | Must | **Scope confirmed by Omar, 2026-09-12**: users photograph or film an observation (permaculture, nature, flowers, mushrooms, etc.), pin it to the map, and other users can visit that same location for a similar experience. Named "Observer" as the actual tab/feature identity, not just a generic "GPS pins" backend concept. Omar flagged this phase as coming up soon in priority. Foundation for biodiversity tool, mushroom map, and community map below — build the pin system once, reuse everywhere. Not started. |
...
**Weeks 20-27** (~early Jan – late Feb 2027) · Platform: Both (mobile-first) · 18 items
...
| Soil Types reference pages | New (Omar, 2026-09-12) | both | Should | A real, currently-unplanned gap: `SoilCard`/RainAdvisor already classify real WRB soil types (ISRIC SoilGrids) and derive Hydrologic Soil Groups, but nothing explains what a given soil type actually means for gardening/farming. Cheap relative to other Phase D items — the classification logic already exists (`soilService.js`, `rainAdvisorService.js`'s `estimateHydrologicSoilGroup`), this is content built on top of it, not new data collection. |
| Climate Zones reference pages | New (Omar, 2026-09-12) | both | Should | Same shape as Soil Types: `smartClimateAdvisor.js` already classifies every location into one of 5 climate zones × 3 elevation bands with real plant/guild recommendations per zone, but there's no standalone page explaining the classification system itself. Reuses `CLIMATE_INFO`/`ELEVATION_INFO` already defined in that file as the content backbone. |
| Field Calculators (folded in: Energy Calculators + new categories) | List B | web | Should | **COMPLETE, 2026-09-13 — 26 of 26 calculators live** (`swales-services` `111ddcb`..`290d5cf`) — independent of both pre-launch gates (plant images, Stripe), built start-to-finish in one session once started. Originally decided 2026-08-27 as earthworks-sizing tools only; Energy Calculators (previously its own separate row) folded in the same day, plus three net-new categories (Vertical Farming, Microgreens, Aquaculture/Aquaponics) kept under this one line rather than given their own roadmap rows. Each calculator is a standalone, manual-entry tool (no map/pin/API dependency — reliability over convenience, and keeps this free tool genuinely separate from the gated map-driven advisors per `feedback_keep_free_tools_separate_from_gated`), using only real, citable formulas, verified by hand before shipping (same bar as RainAdvisor). **Tier 1** (`64f56dd`): swale volume/dimensions, terrace spacing (bench-terrace VI, not cut-fill — see status.md), mulch volume, rainwater tank sizing, solar PV array sizing, hydro power output. **Tier 2** (`940cb69`): compost C:N ratio, pond/dam volume, soil amendment rate, battery bank sizing, inverter sizing, wind turbine output, solar/wind ROI-payback. **Tier 3** (`8f2d4ad`): seed rate/plant population, DLI, nutrient dosing (redesigned around proportional strength scaling, not EC/PPM — see status.md for the real scale-ambiguity reason), vertical farm tier yield, microgreens seed density, microgreens harvest planner, fish stocking density, aquaponics fish-to-plant ratio, dissolved oxygen/aeration, grazing stocking rate (AUM). **Tier 4** (`290d5cf`): biogas potential, Keyline plow line layout (reframed from "spacing calculator" — research confirmed spacing itself has no calculable formula, it's an equipment choice; renamed and rescoped to the one genuinely calculable piece, see status.md), hybrid off-grid system load balancer. Every calculator page ships with `generateMetadata` + `SoftwareApplication`/`FAQPage` JSON-LD schema plus a real SEO/GEO article (explanatory sections + direct-Q&A FAQ list) underneath the tool itself, same pattern as the blog, plus one shared `/field-calculators` hub page covering what the tools are, why they matter, and how to measure the inputs. `roadmap_backlog.xlsx`'s matching rows (Field Calculators, Energy Calculators, plus the two A2 rows it subsumed — ROI/payback and Amendment calculator) all updated to Done, see status.md. Hub cards also got a green hover state (`4b2620f`) matching the existing brand color. |
