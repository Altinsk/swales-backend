...
| Pinpoint global data extraction to user (auto site data from location pin) | List A – #0 | backend | Must | **In progress, 2026-08-27.** Built a shared `getOrFetch` site-data cache (`swales-services/src/lib/map/siteDataCache.js`) so the same dataset for the same pin is never re-fetched twice in one session — wired into solar, soil, wind planning, water stress, flood risk, elevation, geocode, and precipitation (6 of 9 datasets), plus the "full report" PDF generator, which now also always computes Sun position fresh (previously silently missing from the report unless the user had visited the Sun Tracking tab first). **Explicitly excluded, per Omar's call**: weather forecast — deliberately not cached or wrapped, since it changes constantly and doesn't belong in a saved report anyway. **Not yet built**: contour-in-report (reuse an already-drawn rectangle, or prompt for an interval and auto-generate one). **To test before final validation** — see `status.md`'s dated entry for the checklist; needs a logged-in click-through against the deployed backend, not yet done. |
