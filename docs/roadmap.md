...
| Plant & element database (region-aware, tagged) | List A – #1 / Phase 1 | backend | Must | Schema + 171-species seed delivered; companion-planting data, subcategory vocabulary cleanup, and a new derived `layer` field (canopy/sub-canopy/shrub/herbaceous/root/groundcover/vine/cover_crop/fungal) are all functionally complete and already applied/re-seeded on the Neon **dev branch** (2026-08-24/25) — the code for the `layer` field specifically is still sitting uncommitted locally, not because it's unfinished but because it's tied to the paused item below. Remaining, and **paused since 2026-08-25**: full icon population — the actual canvas merge (wiring `/api/elements` into the designer palette) is blocked on real per-species images, which don't exist yet. See `future-concerns.md` item 11 for the full reason and what unblocks it. |
