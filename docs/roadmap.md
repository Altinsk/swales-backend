...
| Specialized Data Package (GeoJSON/CSV/KML/DXF export, one-time purchase) | New (Omar, inspired by permalogica.com/pricing, 2026-09-12) | web | Could | **Not built — CAD (DXF) piece spiked and validated 2026-09-12** (see `status.md`), library confirmed (`@tarikjabiri/dxf`). Distinct product from Core paid: a one-time purchase (not subscription — this is a single deliverable, not ongoing access), aimed at professionals (engineers/designers) who want to import real site geometry into their own CAD/GIS tools, not homesteaders. Recommended v1 scope: GeoJSON + CSV + KML + DXF contour export, all achievable from data already computed today. The AI Q&A piece ("Ada" equivalent) — recommended as a guided/rules-based Q&A instead of an actual LLM (cheaper, deterministic, no hallucination risk on real earthworks advice) — **built, 2026-09-12**: `reportQAService.js` + `AskAboutSite.jsx`, 5 topics (Solar/Wind/Irrigation/Swales & Drainage/Overall Suitability), wired layer-agnostically into `LayerDataPanel.jsx`. Not gated. See `status.md`'s 2026-09-12 entry. |
