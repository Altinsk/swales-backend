...
| Specialized Data Package (GeoJSON/CSV/KML/DXF export, one-time purchase) | New (Omar, inspired by permalogica.com/pricing, 2026-09-12) | web | Could | **Not built — CAD (DXF) piece spiked and validated 2026-09-12** (see `status.md`), library confirmed (`@tarikjabiri/dxf`). Distinct product from Core paid: a one-time purchase (not subscription — this is a single deliverable, not ongoing access), aimed at professionals (engineers/designers) who want to import real site geometry into their own CAD/GIS tools, not homesteaders. Recommended v1 scope: GeoJSON + CSV + KML + DXF contour export, all achievable from data already computed today. Explicitly **not** the AI Q&A piece ("Ada" equivalent) — see the monetization/Permalogica notes in `status.md`: that's separate, real LLM-integration work, deliberately not recommended as a v1 requirement (a guided/rules-based Q&A reusing the existing advisory engines' insight logic is the recommended alternative — see `status.md`'s 2026-09-12 entry). |
