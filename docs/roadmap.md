...
| Gate the single-pin map's analysis calls behind an explicit "Analyze" action | Added 2026-09-05 (Omar) | web | Must | **Added as a prerequisite for Phase A2's Site comparison row, blocking it until this ships.** Investigated while designing Site comparison's own "nothing costly fires without an explicit action" model: found the *existing* single-pin map (`MapComponent.jsx`) has the identical problem today — every pin placement (map click, drag, "My location", typed lat/lng) fires the active layer's real analysis call (`fetchSolarData`/`fetchAllWindData`/`fetchSoilData`/etc.) immediately, with zero debounce or confirmation anywhere in the code, so idle browsing/mis-clicks/a bad geolocation guess/a typo'd coordinate all cost a real call today. Fix: placing/moving a pin stays free (still resolves the cheap place-name/elevation preview); the active layer's real fetch never auto-fires, moved instead behind a per-category "Analyze" action (a toolbar button plus an in-panel "ready to analyze" prompt where the result card would normally render) — matches today's existing per-tab fetch scoping, doesn't introduce a bulk "analyze everything" (the existing Download Site Report button already covers that case). Also fixes two real bugs found along the way: "My location" duplicates the geocode/elevation call on the Altitude layer, and has no branch at all for wind data. Full design (files touched, verification steps) in `status.md`'s 2026-09-05 entry. Not yet built. |
